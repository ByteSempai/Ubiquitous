...
namespace dotFlex.Scheduling
{
    public class Trigger : IComparable
    {
        ...
        private int _misfireInstruction = dotFlex.Scheduling.MisfireInstruction.InstructionNotSet;
        ...
        public void UpdateAfterMisfire()
        {
            ...
            if (instr == dotFlex.Scheduling.MisfireInstruction.SmartPolicy)
            {
                if (RepeatCount == 0)
                {
                    instr = dotFlex.Scheduling.MisfireInstruction.FireNow;
                }
                else if (RepeatCount == RepeatIndefinitely)
                {
                    instr = dotFlex.Scheduling.MisfireInstruction.RescheduleNextWithRemainingCount;

                }
                else
                {
                    instr = dotFlex.Scheduling.MisfireInstruction.RescheduleNowWithExistingRepeatCount;
                }
            }
            else if (instr == dotFlex.Scheduling.MisfireInstruction.FireNow && RepeatCount != 0)
            {
                instr = dotFlex.Scheduling.MisfireInstruction.RescheduleNowWithRemainingRepeatCount;
            }

            if (instr == dotFlex.Scheduling.MisfireInstruction.FireNow)
            {
                SetNextFireTime(DateTime.UtcNow);
            }
            else if (instr == dotFlex.Scheduling.MisfireInstruction.RescheduleNextWithExistingCount)
            {
                NullableDateTime newFireTime = GetFireTimeAfter(DateTime.UtcNow);
                SetNextFireTime(newFireTime);
            }
            else if (instr == dotFlex.Scheduling.MisfireInstruction.RescheduleNextWithRemainingCount)
            {
                NullableDateTime newFireTime = GetFireTimeAfter(DateTime.UtcNow);
            ...
            else if (instr == dotFlex.Scheduling.MisfireInstruction.RescheduleNowWithExistingRepeatCount)
            {
            ...
            else if (instr == dotFlex.Scheduling.MisfireInstruction.RescheduleNowWithRemainingRepeatCount)
            {
                DateTime newFireTime = DateTime.UtcNow;
                int timesMissed = ComputeNumTimesFiredBetween(_nextFireTimeUtc, newFireTime);
